...
class IngestLoop:
    ...
    def run(self, start_date, end_date, skip_ingest):
...
def main():
    assert len(sys.argv) == 5, (
            ...
            ' <end_date> <path_to_config> <skip_ingest>'.format(os.path.basename(__file__)))

    start_date = pd.Timestamp(sys.argv[1],tz='Etc/UTC')
    end_date = pd.Timestamp(sys.argv[2],tz='Etc/UTC')
    config_file = sys.argv[3]
    skip_ingest = sys.argv[4]

    ingest_looper = IngestLoop(config_file)
    ingest_looper.run(start_date, end_date, skip_ingest)
